...
class AppLocalizations {
  ...
  Future<bool> load() async {
    final String jsonLang = await rootBundle
    ...
    _localizedStrings = langMap.map((key, value) {
      if (value is Map) {
        return MapEntry(
            key, value.map((key, value) => MapEntry(key, value.toString())));
      } else {
  ...
  String translate(String key1) {
    return _localizedStrings[key1] ?? '';
  }
